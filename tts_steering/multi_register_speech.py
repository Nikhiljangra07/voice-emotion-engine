...
2. AUTO-RETRY CERTIFICATION (fixes draw noise): synthesize all acts, judge
   ...
   attempt a ledger row (indextts2-multi-register).

Run:  .venv_tts/bin/python tts_steering/multi_register_speech.py
Play: afplay tts_steering/out/user_speech/multi_register_speech.wav
"""
...
P_NEU = str(ROOT / "own_voice/reg_neutral.wav")
P_JOY = str(ROOT / "own_voice/reg_joy.wav")
P_SAD = str(ROOT / "own_voice/reg_sad.wav")
...
def main():
    ...
    for rnd in range(1, MAX_ROUNDS + 1):
        ...
        for i in pending:
            ...
            wav = OUT / f"mr_a{i+1}_{emo}_t{rnd}.wav"
        ...
        for (i, emo, wav), m in zip(meta, res):
            ...
            d = dist(m, emo)
            n += 1
            rows.append([n, "indextts2-multi-register", emo,
    ...
    concat = OUT / "_concatmr.txt"
    concat.write_text("\n".join(lines[:-1]))
    full = OUT / "multi_register_speech.wav"
