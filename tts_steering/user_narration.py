...
Every paragraph judged + ledgered (indextts2-user-narration). Judge frozen.

Run:  .venv_tts/bin/python tts_steering/user_narration.py
Play: afplay tts_steering/out/user_speech/narration_2min.wav
"""
...
def main():
    ...
    concat = OUT / "_concatn.txt"
    concat.write_text("\n".join(lines[:-1]))
    full = OUT / "narration_2min.wav"
    ...
    for (i, w), m in zip(meta, res):
        n += 1
        rows.append([n, "indextts2-user-narration", "narration",
