"""P5A — FEAR OPTIMIZATION (EXPLORATORY): evolution in the native emovec space.

Third Phase-5 run, protocol identical to p5a_joy/sad (pre-registered).
EXPLORATORY: the judge's fear blind spot is documented (its 37 historical
fear verdicts came mostly from sad/happy vectors, not the afraid bank). A
null result here is a finding about the judge, not a failure.
  * TRAIN sentences: s12 + s13 (the exclaim carriers — the only P4.6
    category with fear-affine content). 2 draws per candidate.
  * Score per clip: 2·(judge==fear)·confidence + max(0, 0.5 − dVAD) against
    the MSP fear centroid (−0.21, 0.51, −0.01) — derived from 794 Train
    labels with the established (EmoVal−4)/3 formula.
  ...
  * Hull (identical): coef in [-0.6, +1.0], sum|c| <= 1.6, sum c in
    [-0.2, +1.1]. Init mean = the native afraid bank + a pinch of surprise
    (fear's acoustic kin): [0, 0, 0, 0.6, 0, 0, 0.15, 0].
  * HELD-OUT one-shot (no iteration): best vector on s15 (question, unseen by
    this optimizer) + two NEVER-SEEN fear-affine sentences + one flat
    control (s03).
  * Judge frozen. Every clip a ledger row (indextts2-p5a-fear). Cap:
    160 optimization clips + 8 held-out.

Run:  .venv_tts/bin/python tts_steering/p5a_fear_opt.py
"""
...
FEAR_C = (-0.21, 0.51, -0.01)

TRAIN = {"s12": "Watch out, the glass is about to fall!",
         "s13": "That is exactly what I was afraid of."}
HELDOUT = {"s15": "Did you hear what happened at the station?",
           "h05": "I think someone is following us.",
           "h06": "Please tell me everyone got out in time.",
           "s03": "The report is on the desk in the main office."}  # control

MU, LAM, GENS = 3, 8, 10
INIT_MEAN = [0.0, 0.0, 0.0, 0.6, 0.0, 0.0, 0.15, 0.0]
INIT_SIGMA = [0.12] * 8
...
def score_clip(m):
    d = ((m["V"] - FEAR_C[0]) ** 2 + (m["A"] - FEAR_C[1]) ** 2
         + (m["D"] - FEAR_C[2]) ** 2) ** 0.5
    s = (2.0 * m["conf"] if m["family"] == "fear" else 0.0) \
        + max(0.0, 0.5 - d)
    return s, d
...
def main():
    ...
    def evaluate(tag, cands, sents_draws):
        ...
        for ci, c, sid, dr, wav in meta:
            ...
            if wav not in ledgered:
                n += 1
                hit = m["family"] == "fear"
                rows.append([n, "indextts2-p5a-fear", "fear",
    ...
    for g in range(1, GENS + 1):
        ...
        if g == 1:
            cands[0] = clamp_hull(list(INIT_MEAN))  # anchor the afraid bank
        ...
        sigma = [min(max(s * (1.2 if improved else 0.85), 0.03), 0.3)
                 for s in sigma]
        fear_hits = sum(
            1 for ci in range(LAM) for sid, dr in [("s12", 0), ("s13", 1)]
            if results.get(str(OUT / f"g{g:02d}_c{ci}_{sid}_d{dr}.wav"),
                           {}).get("family") == "fear")
        print(f"gen {g:2d}: fear verdicts {fear_hits}/16  "
    ...
    for sid in HELDOUT:
        for dr in (0, 1):
            ...
            print(f"  {sid} draw{dr}: {m['family']}@{m['conf']:.0%} "
                  f"V={m['V']:+.2f} {'FEAR ✓' if hit else ''}")
    print(f"\nheld-out fear verdicts (excl. control): {hits}/6")
